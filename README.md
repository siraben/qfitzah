...
shape. The newest checked step runs root-slot tracing and scan-forwarding after
allocation overflow, then retries allocation after the copied graph.
...
- **Allocation and GC path**: the staged examples cover writable data, pair
  ...
  checked root-table allocation retry, checked root-table scan-forwarding
  retry, runtime atom copying, and recovered byte output. These are still
