...
self-hosted compiler/runtime stack.

It is not self-hosting yet. The current work is to collapse the Stage 5 garbage
collector proofs toward one reusable collector interface. The newest checked
path starts allocation at `HeapLimit`, traps the first checked allocation into a
root-table recovery path, copies live roots, optionally scan-forwards the copied
graph, then retries allocation after the copied data.

The important audit point: most `stage5-*` files are proof fixtures, not real
bootstrap stages. They demonstrate pieces of the future runtime under current
seed limits.
...
There are only four real rungs today:
...
Everything named `*-ext.qf1` is an overlay. Overlays are not independent
languages; they are local rule packs used when a fixture would otherwise exceed
the seed runtime's finite arithmetic/address tables or practical source-size
budget. When a fixture would load too many rules at once, the tests split the
pipeline:
...
- keep `qfasm2`, `qfasm3`, and `qfc4` as named compiler/assembler rungs
- move repeated Stage 5 behavior into shared compiled routines
- keep overlays small and explicitly documented as budget workarounds
- replace fixture-specific collector code with one root-table collector path
- only call the stack self-hosting after a compiler/runtime rebuilds itself
  byte-for-byte
...
Stage 5 is active but incomplete. It is best read as a set of checked subsystem
proofs that are being collapsed into a runtime/compiler, not as a complete
runtime.

What works:

- the seed reader supports multi-line parenthesized records, comments,
  EOF-balanced records, and explicit `(Rule pattern replacement)` records
- qfc4 can compile focused byte-output paths, normal printers, allocation
  routines, dispatch tables, and small optimizer rewrites
- GC fixtures cover checked allocation, root copying, list/tree/object copying,
  forwarding for sharing and cycles, scan-forwarding, root-table tracing,
  runtime atom copying, and recovered byte/normal output
- dispatch and optimization are complete enough to mark roadmap Tasks 5.2 and
  5.3 done as staged proofs

What is being cleaned up now:

- checked root-table fixtures now share the overflow, retry, and root-table
  reset qfc4 surface in
  [bootstrap/qfc4-checked-root-table-common-ext.qf1](bootstrap/qfc4-checked-root-table-common-ext.qf1)
- the smaller checked root-table fixture reuses the shared `TraceRoots`
  routine from
  [bootstrap/qfc4-root-table-forwarding-ext.qf1](bootstrap/qfc4-root-table-forwarding-ext.qf1)
- the checked scan-forwarding fixture still uses a local trace-to-scan
  trampoline because adding the wider final qfasm overlay pushes that fixture
  over the current seed source budget
...
The detailed roadmap is in [plan.md](plan.md), the remaining self-hosting gaps
are in [bootstrap/self-hosting-gap.md](bootstrap/self-hosting-gap.md), and the
