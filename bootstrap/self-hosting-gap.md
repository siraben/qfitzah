...
- `stage5-checked-root-table-forwarding-gc-qfc4.qf1` connects checked
  allocation to the root-table collector shape. `HeapNext` starts at
...
- `stage5-dispatch-chain-miss-qfc4.qf1` exercises the same linked-table
  ...
  with status `9`.
- `stage5-dispatch-runtime-chain-qfc4.qf1` moves the dispatched argument
  signature from hardcoded comparison immediates into runtime data cells. The
  qfc4 source compiles those class cells plus the linked table, then generated
  code loads the actual classes, compares table entries against those runtime
...
- `stage5-print-list-qfc4.qf1` starts the normal-printer path. It compiles a
...
- `stage5-print-nil-and-list-tail-qfc4.qf1` merges the nil branch with the
  tail-recursive two-element list printer. It is generated by
...
- `stage5-known-match-opt-qfc4.qf1` adds a focused known-match proof for that
  optimizer: a compile-time zero match test is reduced to the selected arm, so
  qfasm3 never sees the runtime `IfZero` tree for that fixture.
