...
- `stage5-multi-root-forwarding-gc.qf1`
  - generated by `tools/generate_stage5_multi_root_forwarding_gc.py`
  - reuses the direct scan-forwarding field handlers and wide-branch extension
  - proves a two-slot root set where both roots begin at the same old object,
    the second root is rewritten through the first copy's forwarding marker,
    and the scan loop still copies the shared cyclic child once
  - overwrites the old root and child, verifies root convergence, child
    sharing, the child self-cycle, and the two-cell `HeapNext` frontier, then
    exits `19`
- `stage5-multi-root-forwarding-gc-qfc4.qf1`
  - generated by `tools/generate_stage5_multi_root_forwarding_qfc4.py`
  - reuses `qfc4-scan-forwarding-ext.qf1` for the field handlers and adds
    `qfc4-multi-root-forwarding-ext.qf1` only for root-set setup/checks
  - loads `qfasm-multi-root-forwarding-ext.qf1`, a deliberately tiny numeric
    extension for the longer backward `ScanCdr` call introduced by the
    multi-root prologue
  - proves the same two-root convergence and copied cyclic child behavior
    through qfc4, exiting `19`
- `stage5-root-table-forwarding-gc-qfc4.qf1`
  - generated by `tools/generate_stage5_root_table_forwarding_qfc4.py`
  - replaces hard-coded root forwarding with a `RootScan` to `RootEnd` slot
    table traversal before the shared scan-forwarding loop
  - uses a tiny `Start` trampoline so the ELF entry remains inside qfasm2's
    common address table while helper procedures stay close to their calls
  - loads `qfasm-root-table-forwarding-ext.qf1` only for the local arithmetic
    range needed by the trampoline and verifies exit status `19`
- `stage5-checked-root-table-forwarding-gc-qfc4.qf1`
  - generated by `tools/generate_stage5_checked_root_table_forwarding_qfc4.py`
    with shared checked overflow, retry, and root-table reset rules from
    `qfc4-checked-root-table-common-ext.qf1`
  - starts `HeapNext` at `HeapLimit`, forcing the first checked allocation to
    overflow before root-table tracing runs
  - calls the shared `TraceRoots` routine from the root-table forwarding
    extension instead of carrying its own trace loop
  - keeps the copied graph to one shared root cell so the fixture proves the
    allocation/collector handoff without exceeding the current qfasm source
    budget
  - retries allocation after the copied root, verifies root convergence and the
    retry cell, and exits through the copied root car (`19`)
- `stage5-checked-root-table-scan-forwarding-gc-qfc4.qf1`
  - generated by `tools/generate_stage5_checked_root_table_scan_forwarding_qfc4.py`
    with shared checked overflow, retry, and root-table reset rules from
    `qfc4-checked-root-table-common-ext.qf1`
  - extends the checked root-table path with the shared scan-forwarding loop,
    so the copied root's shared cyclic child is copied before allocation retry
  - keeps a local trace-to-scan trampoline instead of calling the shared
    `TraceRoots` procedure, because the larger final qfasm pass cannot also
    load the root-table numeric overlay under the current source-size budget
  - deliberately avoids `qfasm-root-table-forwarding-ext.qf1` in the final
    qfasm pass; the extra numeric overlay pushes this larger staged fixture
    past the seed runtime's practical source budget
  - verifies root convergence, the retry frontier, the retry cell, and copied
    child data, exiting `19`
...
- `stage5-checked-scan-forwarding-dynamic-atom-gc.qf1`
  - generated by `tools/generate_stage5_checked_scan_forwarding_dynamic_atom_gc.py`
  - starts `HeapNext` at `HeapLimit`, so the first checked pair allocation
    overflows instead of invoking recovery directly
  - runs scan-forwarding recovery with runtime atom copying, retries allocation
    after the copied pair graph, and checks the retry pair did not overwrite
    copied live data
  - assembles with the heap-check, scan, and wide-branch extensions and exits
    `0`
- `stage5-checked-scan-forwarding-dynamic-atom-gc-qfc4.qf1`
  - generated by `tools/generate_stage5_checked_scan_forwarding_dynamic_atom_qfc4.py`
    with `qfc4-checked-scan-forwarding-dynamic-atom-ext.qf1`
  - lowers the checked overflow, scan-forwarding recovery, runtime atom copy,
    retry allocation, and post-retry verification through qfc4
  - uses `qfasm-stage5-checked-ext.qf1` to keep the checked-allocation compare
    and raw scan branch pass-through local to this fixture
  - verifies exit status `0` through the staged qfc4 -> qfasm3 source, then
    qfasm3/qfasm2 -> ELF pipeline
- `stage5-dispatch-table-qfc4.qf1`
  - generated by `tools/generate_stage5_dispatch_table_qfc4.py` with
    `qfc4-dispatch-ext.qf1` and `qfasm-dispatch-ext.qf1`
  - compiles dispatch-entry data containing two-argument signature records and
    concrete method labels
  - adds only the local `CallEax` indirect-call instruction needed to call a
    method pointer loaded from the table
  - verifies the first table entry is skipped, the second matching method is
    loaded and called, and the generated ELF exits `42`
- `stage5-dispatch-chain-qfc4.qf1`
  - generated by `tools/generate_stage5_dispatch_table_qfc4.py` with
    `qfc4-dispatch-chain-ext.qf1` and `qfasm-dispatch-ext.qf1`
  - keeps the chain-specific qfc4 rules separate from the smaller unrolled
    table fixture to stay inside the seed runtime's practical source budget
  - compiles each entry as signature, method, and next-entry pointer records
  - verifies the loop skips an arg1 miss and an arg2 miss, follows next links,
    indirectly calls the first matching method, and exits `42`
- `stage5-dispatch-chain-miss-qfc4.qf1`
  - generated by the same dispatch-table generator and uses the same optional
    chain dispatch extension
  - compiles a linked table with no matching method
  - verifies the loop skips both entries, reaches `NoEntry`, and exits through
    the dispatch-miss path with status `9`
- `stage5-dispatch-runtime-chain-qfc4.qf1`
  - generated by the same dispatch-table generator with
    `qfc4-dispatch-runtime-chain-ext.qf1` and
    `qfasm-dispatch-runtime-ext.qf1`
  - keeps runtime-argument dispatch rules separate from the immediate-chain
    extension because combining them makes the seed runtime crash in the
    all-in-one pipeline
  - runs as qfc4 -> qfasm3 source, then qfasm3/qfasm2 -> ELF under Qfitzah
  - loads the two actual dispatch classes from runtime data cells, compares
    table signatures with register-register comparisons, skips arg1 and arg2
    misses, indirectly calls the selected method, and exits `42`
- `stage5-dispatch-mutable-runtime-chain-qfc4.qf1`
  - generated by the same dispatch-table generator and reuses the runtime
    dispatch extension pair
  - makes `qfasm-dispatch-runtime-ext.qf1` override the ELF header with a
    writable executable segment and add the one local dword-store instruction
    needed to rewrite a class cell
  - rewrites the second argument class from `2A` to `23` before dispatch, then
    proves the linked-table lookup observes the mutated runtime value by
    selecting the alternate method and exiting `8`
- `stage5-dispatch-mutable-method-chain-qfc4.qf1`
  - generated by the same dispatch-table generator and reuses the staged
    runtime-dispatch pipeline
  - adds a qfc4 statement for rewriting a dispatch payload's method pointer
    with a concrete label address
  - proves the dispatcher loads the method address from table state at call
    time: after mutating both the second argument class and the selected
    payload method, the generated ELF exits `42`; the stale class and stale
    method paths are distinguished by statuses `7` and `8`
- `stage5-optimization-qfc4.qf1`
  - uses optional `qfc4-opt-ext.qf1` rules loaded after qfc4 so optimization
    overrides do not increase the common qfc4 rule load
  - folds `(Add1 (Literal 29))` directly to `MovEbxImm32 2A`
...
- `stage5-tco-qfc4.qf1`
  ...
  - rewrites a final `(CallProc Target)` followed by `(Return)` through the
    existing `TailCallProc` lowering
  - verifies the wrapper emits a direct short jump to `Target` and the ELF
    exits `42`

These optimizer rules are intentionally still in an overlay. Merging them into
common `qfc4.qf1` would be the cleaner architecture, but every common rule is
paid by every large staged fixture under the seed runtime. The task-level proof
for optimization is complete; collapsing the overlay should be done only after
the common source-size budget is widened or generated.
...
- `stage5-print-nil-and-atom-qfc4.qf1`
  - reuses `qfc4-print-nil-ext.qf1` and keeps the combined nil/atom printer
...
- `stage5-print-nil-and-list-tail-qfc4.qf1`
  - generated by `tools/generate_stage5_print_nil_tail_qfc4.py` because the
    merged nil/list source variants are nested enough to be error-prone by hand
